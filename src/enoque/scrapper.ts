...
export const SOURCES: Record<Lang, { url: string; file: string }> = {
  pt: { url: 'https://faithofgod.net/1ENOQUE.html', file: 'output/apocryphal/enoque/raw/1ENOQUE.html' },
  en: { url: 'https://faithofgod.net/1ENOCH.html', file: 'output/apocryphal/enoque/raw/1ENOCH.html' },
}
...
export async function getSource(lang: Lang, force = false): Promise<string> {
  ...
  await mkdir('output/apocryphal/enoque/raw', { recursive: true })
